...
from Params_Config.params_config import xm_per_pix, ym_per_pix
...
def get_fits_by_sliding_windows(binary_img, line_lt, line_rt, img_name='', n_windows=9, display=False):
    """
    Get polynomial coefficients for lane-lines detected in an binary image.

    :param binary_img: input bird's eye view binary image
    ...
    :param display: if True, display intermediate output
    ...
    histogram = np.sum(binary_img[height//2:-30, :], axis=0)
    ...
    if display:
        ...
        ax[0].set_title('lane fitting before')
        ax[1].imshow(out_img)
        ax[1].set_title('lane fitting after')
        ax[1].plot(left_fitx, ploty, color='yellow')
        ax[1].plot(right_fitx, ploty, color='yellow')
        ax[1].set_xlim(0, 1280)
        ax[1].set_ylim(720, 0)
        
        plt.savefig("../output_images/lane_fitting/" + img_name[15:-4] + "_lane_fitting_after.jpg",dpi=300)
        # plt.show()

    return line_lt, line_rt, out_img


def get_fits_by_previous_fits(binary_img, line_lt, line_rt, img_name='', display=False):
    """
    Get polynomial coefficients for lane-lines detected in an binary image.
    ...
    img_fit = np.dstack((binary_img, binary_img, binary_img)) * 255
...
if __name__ == '__main__':
    ...
    for img_test in imgs_raw:
        ...
        # transform_image
        transforming_img, M, Minv = transforming_image(img_binarization, img_name=img_test, display=False)
        line_lt, line_rt, img_out = get_fits_by_sliding_windows(transforming_img, line_lt, line_rt, img_name=img_test, n_windows=9, display=True)
        cv2.imshow("img_out",img_out)
        cv2.waitKey(500)
        cv2.destroyAllWindows()
    print("Lane fitting have finshed")
